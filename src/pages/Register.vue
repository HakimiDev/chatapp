        ...
        <Layout class="w-full sm:w-[70%] h-fit rounded mb-10">
            ...
            <main class="min-w-full mt-2 p-1 flex justify-center items-center">
                <form class="w-[92%]" onsubmit="return false;">
                    ...
                    <InputField
                        :className="(!showPassword && inputsData.password.trim().length && inputsData.password === inputsData.rePassword) ? 'text-green-400' : (!showPassword && inputsData.password.trim().length) ? 'text-red-400' : ''"
                    ...
                    <Button @onClick="handelSignup" :loading="loading" class="min-w-full flex justify-center items-center mt-4">Sign Up</Button>
                    ...
                    <p class="p-1 px-4 text-gray-400 text-center text-lg">Have an account? <a
                            class="cursor-pointer font-semibold transition hover:underline" href="/#login">Signin
                            instead</a></p>
...
const handelSignup = async () => {
    if (!Object.values(inputsData.value).every(e => e.trim().length)) return;
    // Todo check email
    if (inputsData.value.password.trim().length < 8) return;
    if (inputsData.value.password !== inputsData.value.rePassword) return;
